...
为了保证仓库内容清晰、方便组员协作，所有成员在完成自己负责的部分后，都需要同步补充对应的说明文档，而不仅仅是上传代码或文件。

具体要求如下：

### 1. 负责数据集处理的成员
在完成数据下载、清洗、预处理或特征工程后，需要及时更新 `data/README.md`，至少说明：
- 数据集名称；
- 数据来源；
- 原始数据文件名；
- 处理后数据文件名；
...
### 3. 负责实验记录的成员
在完成实验后，需要在 `experiments/` 对应文件夹中记录：
- 使用的数据集；
- 使用的模型；
- 参数设置；
- 实验结果；
...
### 4. 负责报告撰写的成员
在更新报告草稿时，需要将内容整理到 `report/draft/` 对应文件中，并尽量保持结构清晰，方便后续整合。

### 5. 通用要求
所有成员在提交文件时，不仅要上传内容本身，也要保证其他组员能够看懂并复现你的工作。  
如果你新增了数据、脚本、实验结果或报告内容，请同时补充必要说明。
