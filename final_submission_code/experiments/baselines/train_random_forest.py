...
def parameter_grid(task_type: str) -> list[dict[str, Any]]:
    ...
    if task_type == "classification":
        common_grid["class_weight"] = [None, "balanced"]
...
def train_final_and_evaluate(
    ...
    model = make_model(task_type=task_type, params=tuning_result["best_params"])
...
def train_one_dataset(processed_dir: Path, dataset_name: str) -> dict[str, Any]:
    ...
    print(f"\n===== Training Random Forest on {dataset_name} ({task_type}) =====")
    print(
        "split shapes:",
        data["X_train"].shape,
        data["X_val"].shape,
...
def main() -> None:
    ...
    print(f"\nSaved Random Forest results to: {args.output}")
